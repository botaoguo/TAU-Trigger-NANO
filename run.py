...
def run_TriggerTool(_dir, _isMC, _start, _end):
    for _jobid in range(_start, _end + 1):
        _cmd = "nohup python postproc.py --input /eos/cms/store/group/phys_tau/TauFW/nanoV10/Run2_2018/{0}/nano_{1}.root --output ztest_0514_data/{0} --isMC {2} --era 2018 > ztest_0514_data/{0}/nohup_{1}.log 2>&1 &".format(_dir, _jobid, _isMC)
        print(_cmd)
        print("Running now!")
        os.system(_cmd)
        # exit(0)
